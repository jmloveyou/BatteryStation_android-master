...
public class CustomCrashHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        ...
        LogUtil.v("uncaughtException");
        //将一些信息保存到SDcard中
        savaInfoToSD(mContext, ex);

//        MobclickAgent.reportError(mContext,ex);
        //提示用户程序即将退出
        showToast(mContext, mContext.getString(R.string.app_will_exit));
        try {
            thread.sleep(2000);
...
//        ExitAppUtils.getInstance().exit();
    ...
    private String savaInfoToSD(Context context, Throwable ex){
        ...
        if(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)){
            ...
            try{
                fileName = dir.toString() + File.separator + "bs_"+paserTime(System.currentTimeMillis()) + ".log";
